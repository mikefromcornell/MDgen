...
| **Version** | v1.1 — supersedes Draft v1.0 ([original preserved](docs/PRD-original.md)) |
...
| D4 | Scope | **Images + Docs** (PDF/DOCX/TXT/CSV) | Docs libs are small; huge utility gain |
| D5 | Portfolio | **Folder inside MDgen** (`portfolio-context/`) | Owner asked for one repo; same raw-URL linking works (`.../MDgen/main/portfolio-context/portfolio.md`) |
| D6 | Extras in MVP | PWA offline, share-`#md=` link, chunk ZIP, prompt templates | High value / still $0; URL→MD, QR, inbox-Action deferred to roadmap |
...
CSV converters, settings/BYOK panel, themes, shortcuts, a11y, PWA, SEO files.

## 3. Success metrics (targets, unchanged)

| Metric | Target | v1 instrument |
|---|---|---|
| Paste→copy, local OCR 1080p | p50 < 8 s | preprocessing ≤0.5 s + warm worker ~3–6 s |
| Run-rate cost | $0 | Pages + in-repo libs + Stooq keyless; only optional user key |
...
| Code quality | no build step | syntax-checked + 73 passing unit tests on the MD pipeline |
| Portfolio freshness | quotes < 24 h old | weekday cron Action, ~1 min/run (free tier 2,000 min/mo) |
...
Browser ├─ Local: Tesseract.js v7 (WASM worker, tessdata eng in-repo) ┌ default
        ...
        │         mammoth (DOCX) · JSZip (batch/chunk ZIP) · marked+DOMPurify
        └─ AI:    fetch browser→Gemini (x-goog-api-key) / OpenAI · BYOK
portfolio-context/ + .github/workflows/portfolio-quotes.yml (Stooq CSV, weekday cron)
```
...
| Stooq symbol misses (`.us`) | suffix normalizing, `n/a` rows, never corrupts existing files |
| AI free-tier quotas shrink | model name editable; Local OCR unaffected |
| Very large CSV/PDF | 200-row render cap + 50 k parse cap; per-page progress + cancel |
...
| M5 | portfolio-context | ✅ scaffold + Action shipped; runs once repo is public |
